...
<ul>
  ...
  <li><a href="consumer_list.php">Consumer Lists</a></li>
...
$sql = mysqli_query($conn, "SELECT * FROM billing WHERE account_number LIKE '%$search%'");
...
$count = mysqli_num_rows($sql);
    ...
    while($row = mysqli_fetch_assoc($sql))
        {
			...
			<tr>
                    ...
                    <?php
                        if($row['penalty']==0){ ?>
                            <a href="total.php?id=<?php echo $row['id']?>&bill_amount=<?php echo $row['bill_amount']?>" class="alert alert-success"/>Overdue</a>
                        <input type="hidden" name="id" value="<?php echo $row['id']?>">
                      <?php }else{ ?>
                        <a href="total.php?id=<?php echo $row['id']?>" class="alert alert-danger"/>Overdue</a>
    ...
    else
    {
        ...
        <h3><strong>Billing information not found!</strong></h3>
        </div>
